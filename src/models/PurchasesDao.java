...
public class PurchasesDao {
    ...
    //Registrar compra
    public boolean registerPurchaseQuery(int employee_id, double total) {
        String query = "INSERT INTO purchases (employee_id, total, created)"
                + "VALUES (?,?,?)";
        ...
        try {
            ...
            JOptionPane.showMessageDialog(null,"Error al registrar la compra aaa" + e);
            return false;
        }
    }
    ...
    public List listAllPurchasesQuery() {
        ...
        String query = "SELECT id, total, created FROM purchases";
        try {
            conn = cn.getConnection();
            pst = conn.prepareStatement(query);
            rs = pst.executeQuery();
